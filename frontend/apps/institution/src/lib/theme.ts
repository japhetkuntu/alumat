...
// against, so it needs a real address for institution-api. Inside Docker Compose
...
const API_URL = process.env.INSTITUTION_API_INTERNAL_URL || "http://localhost:5100/api/v1";
...
export async function getInstitutionTheme(): Promise<InstitutionTheme | null> {
  try {
    ...
    // slug is read from a cookie instead, mirrored there by the login page.
...
// In :root (light mode), --primary/--info/--chart-1/--accent-sky all
// reference var(--brand-primary), so overriding that one custom property
// alone used to update those automatically. But .dark redefines every one of
// those same names with hardcoded green oklch literals of its own — a fixed
// color completely independent of --brand-primary — so dark mode never
// reflected the institution's color at all. (--success/--accent-emerald and
// --sidebar-primary's default are deliberately fixed colors even in light
// mode — a real "success" green and a fixed navy sidebar — not part of this
// institution-theming chain, so left alone.) Several light-mode tokens are
// also literal hex values, not var() references — --brand-primary-light/-dark,
// --color-background-info, --color-text-info, --color-border-info, --ring,
// --sidebar-ring. Deriving the full palette from one seed color (see
