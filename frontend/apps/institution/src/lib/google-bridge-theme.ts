 ...
 * to the Google sign-in bridge (a separate page, on the member app's fixed
 * base domain, with no tenant context of its own) as URL params, so its
 * branding panel can render in this institution's actual colors instead of
 * the generic platform default. See apps/member/src/app/google-auth/page.tsx.
 */
export function useGoogleBridgeTheme() {
  return useQuery({
    queryKey: ["google-bridge-theme"],
    queryFn: async () => {
      const res = await institutionClient.get<{ data: GoogleBridgeTheme }>("/public/institution/theme");
