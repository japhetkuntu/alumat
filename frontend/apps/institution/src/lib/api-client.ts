...
// (e.g. greenfield.yourplatform.example/api/v1/...), and nginx proxies that
// path to institution-api while preserving the Host header — which is what
...
// browser is actually on. Set NEXT_PUBLIC_INSTITUTION_API_URL only for setups that
// deliberately want a fixed cross-origin API host.
const API_URL = process.env.NEXT_PUBLIC_INSTITUTION_API_URL || "/api/v1";
...
export const institutionClient = createClient(API_URL);
