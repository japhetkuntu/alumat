...
export default function StoreCallbackPage() {
  ...
  const [message, setMessage] = useState(reference ? "" : "No order reference found");
      ...
      <Card className="w-full max-w-md text-center animate-in fade-in slide-in-from-bottom-3 duration-500">
        <CardHeader className="items-center pb-2">
          ...
          <CardTitle className="text-[22px] mt-1">
            {status === "success" ? "Order confirmed" : status === "error" ? "We couldn't confirm this payment" : "We're checking your payment"}
          </CardTitle>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Button className="flex-1 font-semibold" onClick={() => router.push("/store/orders")}>
              My orders
            </Button>
          </div>
        </CardContent>
