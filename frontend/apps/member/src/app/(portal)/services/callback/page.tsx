...
import { getServiceRequestStatus } from "@/lib/member-api";
import { handleApiError } from "@/lib/api-client";

export default function ServiceRequestCallbackPage() {
  ...
  const [message, setMessage] = useState(reference ? "" : "No request reference found");
  const [counter, setCounter] = useState(10);

  const statusLabel = useMemo(() => {
    switch (status) {
      case "loading": return "Verifying payment...";
  ...
  const pollStatus = useCallback(async () => {
    ...
    setStatus("loading");
    try {
      const response = await getServiceRequestStatus(reference);
      const normalized = response.paymentStatus?.toLowerCase() ?? "unknown";

      if (normalized === "successful") {
        setStatus("success");
        setMessage(response.message ?? "Your payment was successful and your request has been submitted.");
        return;
      }
      ...
      setStatus("error");
      setMessage(response.message ?? `Payment status: ${response.paymentStatus}`);
    } catch (err) {
      setStatus("error");
      ...
      <Card className="w-full max-w-md text-center animate-in fade-in slide-in-from-bottom-3 duration-500">
        <CardHeader className="items-center pb-2">
          ...
          <CardTitle className="text-[22px] mt-1">
            {status === "success" ? "Request submitted" : status === "error" ? "We couldn't confirm this payment" : "We're checking your payment"}
          </CardTitle>
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Button className="flex-1 font-semibold" onClick={() => router.push("/services/requests")}>
              My requests
            </Button>
          </div>
        </CardContent>
