    ...
    ILogger<BusinessDirectoryService> logger) : IBusinessDirectoryService
{
    public async Task<IApiResponse<PgPagedResult<BusinessListingDto>>> GetListingsAsync(BusinessListingFilter filter)
    {
        try
        {
            ...
            // the listing at creation time — refresh both from the live Member
            // records here (one batched lookup, not one query per row) so a later
            // name/email change actually shows up. Falls back to the snapshot if the
            // member has since been deleted (or the listing is admin-created, with no
            // MemberId at all).
    ...
    public async Task<IApiResponse<List<BusinessListingDto>>> GetMyListingsAsync(string memberId)
    {
        try
        {
            ...
            var dtos = listings.Select(l => l.ToDto()).OrderByDescending(l => l.CreatedAt).ToList();
