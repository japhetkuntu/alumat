    ...
    ILogger<MemberAuthService> logger) : IMemberAuthService
{
    ...
    /// <summary>
    /// The current request's own scheme+host — e.g. "https://greenfield.yourplatform.example".
    /// Email links (verify, reset) must point back to whichever institution's
    /// member portal the request actually came from, not a fixed domain: this
    /// platform hosts any number of institutions, each on its own host, and
    /// nginx's same-origin routing means the API's own request Host is
    /// exactly the member frontend's host. Must be captured synchronously,
    /// before any `_ = FireAndForgetAsync(...)` dispatch — HttpContext isn't
    /// safe to read once the request that owns it may have already completed.
    /// </summary>
    ...
    private async Task SendVerificationLinkEmailAsync(string firstName, string email, string token, string baseUrl)
    {
        try
        {
            ...
            await emailService.SendEmailAsync(new SendEmailRequest
            {
                ...
                TemplateVariables = new { first_name = firstName, verify_url = link, brand_color = brandColor },
            });
        }
        catch (Exception e)
        {
    ...
    private async Task SendResetPasswordEmailAsync(string firstName, string email, string token, string baseUrl)
    {
        try
        {
            ...
            await emailService.SendEmailAsync(new SendEmailRequest
            {
                ...
                TemplateVariables = new { first_name = firstName, reset_url = link, brand_color = brandColor },
            });
        }
        catch (Exception e)
        {
    ...
    private async Task SendOtpEmailAsync(string firstName, string email, string otp)
    {
        try
        {
            await emailService.SendEmailAsync(new SendEmailRequest
            {
                ...
                TemplateVariables = new { first_name = firstName, otp_code = otp, brand_color = await GetBrandColorAsync() },
            });
        }
        catch (Exception e)
        {
